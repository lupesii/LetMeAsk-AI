...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: 'Transcreva o áudio para português do Brasil. Seja preciso e natural na transcrição. Mantenha a pontuação adequada e divida o texto em parágrafos quando for apropriado. Não retorna nada além do que a transcrição do audio enviado',
      },
      {
        inlineData: {
          mimeType,
...
export async function generateAnswer(
  ...
  const prompt = `
    ...
    - Se a resposta não for encontrada no contexto, não responda nada;
    - Seja objetivo;
